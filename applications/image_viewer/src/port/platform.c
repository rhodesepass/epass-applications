...
{
    iv_platform_t *platform = lv_display_get_user_data(display);
...
iv_key_t iv_platform_read_key(iv_platform_t *platform)
{
    ...
    while(read(platform->input_fd, &event, sizeof(event)) == sizeof(event)) {
        ...
        switch(event.code) {
        case KEY_1: case KEY_UP: case KEY_LEFT: return IV_KEY_PREV;
        case KEY_2: case KEY_DOWN: case KEY_RIGHT: return IV_KEY_NEXT;
        case KEY_3: case KEY_ENTER: return IV_KEY_ENTER;
        case KEY_4: case KEY_ESC: case KEY_BACK: return IV_KEY_BACK;
        default: break;
        }
    }
    return IV_KEY_NONE;
